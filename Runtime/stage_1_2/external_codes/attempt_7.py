...
def pred(
    ...
    ll_sample = EinsumNetwork.eval_loglikelihood_batched_autocast(model, zero_input,batch_size=12000) ######  batch size
    pred_zero = ll_sample
    ...
    for idx in tqdm(range(loader.num_batches)):
        ...
        ll_sample = EinsumNetwork.eval_loglikelihood_batched_autocast(model, next(loader),batch_size=12000) ######  batch size
...
def quantize_func(model):
    data_dir = '/path/to/CDT_project/train_test'
    ...
    train_x = torch.from_numpy(binary_array).to(torch.device('cuda'))
    ...
    idx_batches = torch.randperm(train_N).split(100)
...
if __name__ == "__main__":
    ...
    if args.sequence_length == 1000:
        ...
        quantize_func(model)   ### to quantize
    else:
        print("DEBUGGING")
        # load the model
        model = load_model_eval(
