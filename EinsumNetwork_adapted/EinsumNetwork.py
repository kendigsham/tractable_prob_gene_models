...
def eval_loglikelihood_batched(einet, x, labels=None, batch_size=100):
    ...
    with torch.no_grad():
        idx_batches = torch.arange(0, x.shape[0], dtype=torch.int64, device=x.device).split(batch_size)
        ll_total = 0.0
        # list_ll_individual=[]    ### 26 Feb  added
        ll_individual = torch.empty((x.shape[0],1), device=x.device)
        ...
        for batch_count, idx in enumerate(idx_batches):
            ...
            ll_individual[idx] = ll_sample    ### 13 Mar 2024 added
        #     ll_total += ll_sample.sum().item()
        #     list_ll_individual.append(ll_sample)   ### 26 Feb  added
        # ll_individual = torch.cat(list_ll_individual)    ### 26 Feb  added
        return ll_individual      ### 26 Feb  added returning ll_sample 13 Mar deleted ll_total


def eval_loglikelihood_batched_autocast(einet, x, labels=None, batch_size=100): ## trying autocast
    """Computes log-likelihood in batched way."""
    with torch.no_grad():
        idx_batches = torch.arange(0, x.shape[0], dtype=torch.int64, device=x.device).split(batch_size)
        ll_total = 0.0
        ...
        for batch_count, idx in enumerate(idx_batches):
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                ...
                ll_individual[idx] = ll_sample
            # ll_total += ll_sample.sum().item()
        #     list_ll_individual.append(ll_sample)
        # ll_individual = torch.cat(list_ll_individual)
        return ll_individual

def eval_loglikelihood_batched_orig(einet, x, labels=None, batch_size=100):
    """Computes log-likelihood in batched way."""
    with torch.no_grad():
        idx_batches = torch.arange(0, x.shape[0], dtype=torch.int64, device=x.device).split(batch_size)
        ll_total = 0.0
